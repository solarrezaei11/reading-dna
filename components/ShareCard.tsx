...
type Props = { dna: DnaProfile; bookCount: number };
...
export default function ShareCard({ dna, bookCount }: Props) {
  ...
  const download = (blob: Blob) => {
    ...
    anchor.href = url;
    anchor.download = "reading-dna.png";
    anchor.click();
  ...
  const copyToClipboard = async () => {
    ...
    try {
      ...
      if (!navigator.clipboard?.write || typeof ClipboardItem === "undefined") {
        download(blob);
        setShareStatus("Clipboard sharing is unavailable. The image was downloaded instead.");
        return;
      }
      try {
        ...
        download(blob);
        setShareStatus("Clipboard permission was denied. The image was downloaded instead.");
      }
    } catch {
  ...
  const pace = clamp(dims.pacing_preference ?? 5, 0, 10);
  const prose = clamp(dims.prose_density ?? 5, 0, 10);
  const paceLabel = pace > 6 ? "Fast-paced" : pace < 4 ? "Slow burn" : "Varied pacing";
  const proseLabel = prose > 6 ? "Dense prose" : prose < 4 ? "Breezy" : "Balanced prose";
  ...
  return (
    <section className="space-y-3">
      ...
      >
        ...
        <div className="relative p-7 space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px]" style={{ color: "rgba(160,136,112,0.6)" }}>{new Date().getFullYear()}</span>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="text-[1.65rem] leading-tight" style={{ fontFamily: "var(--font-dm-serif)", color: "#f2ece0", fontStyle: "italic" }}>
              {dna.reader_archetype ?? "Reading profile"}
            </div>
            <div className="text-xs leading-relaxed" style={{ color: "rgba(200,185,160,0.85)" }}>{dna.one_liner ?? dna.taste_summary}</div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            {[{ val: bookCount, label: "Books" }, { val: dna.avg_rating?.toFixed(1) ?? "—", label: "Avg rating" }, { val: `${Math.round(clamp(dims.fiction_ratio ?? 50, 0, 100))}%`, label: "Fiction" }].map(({ val, label }) => (
              <div key={label}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {(dna.top_themes ?? []).slice(0, 5).map((t) => (
          ...
          <div className="flex flex-wrap gap-x-2 text-[10px]" style={{ color: "rgba(160,136,112,0.6)" }}>
            <span>{proseLabel}</span><span>·</span><span>{paceLabel}</span><span>·</span><span>Depth {Math.round(clamp(dims.intellectual_depth ?? 5, 0, 10))}/10</span>
          </div>
        </div>
      </div>
      ...
      <p aria-live="polite" className="text-xs" style={{ color: "var(--text-3)" }}>{shareStatus}</p>
    </section>
  );
}
