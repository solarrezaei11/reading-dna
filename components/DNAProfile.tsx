...
type Props = { dna: DnaProfile };

import type { DnaProfile, TasteDimension } from "@/lib/types";
import { clamp } from "@/lib/recommendations";

const DIM_LABELS: Record<Exclude<TasteDimension, "fiction_ratio">, [string, string]> = {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="text-3xl leading-tight" style={{ fontFamily: "var(--font-dm-serif)", color: "var(--text-1)", fontStyle: "italic" }}>
            {dna.reader_archetype ?? "Your reading profile"}
          </div>
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-2)" }}>{dna.taste_summary ?? "Your reading history is being summarized."}</p>
        </div>
        ...
        >
          ...
          {(Object.entries(DIM_LABELS) as Array<[keyof typeof DIM_LABELS, [string, string]]>).map(([key, [lo, hi]]) => {
            const val = clamp(dims[key] ?? 5, 0, 10);
            return (
